...
export function createAtmosphere(THREE, { scene, renderer, camera, sun, hemi }) {
  ...
  settings.innerHTML = '<div class="world-settings-head"><strong>Settings</strong><button type="button" id="world-settings-close" aria-label="Close settings">×</button></div><label class="world-setting-row"><span>World sound</span><button type="button" id="world-sound" aria-pressed="false">Off</button></label><div class="world-setting-row"><span>Graphics</span><strong id="world-quality-fixed">HIGH QUALITY</strong></div><div class="world-setting-row"><span>Version</span><strong>1.0.2 · V103.0</strong></div><button type="button" id="blocked-accounts" class="settings-fullscreen">Blocked accounts</button><output id="world-audio-status" role="status"></output>';
